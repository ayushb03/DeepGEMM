"""
FP16 to FP32 GEMM operations for DeepGEMM.

This module provides optimized GEMM kernels that operate on FP16 inputs
...
# Template for FP16->FP32 GEMM kernel
FP16_FP32_GEMM_TEMPLATE = """
#include <cuda_fp16.h>
...
extern "C" __global__ void ${kernel_name}(
    ...
    if (m_idx < M && n_idx < N) {
        ...
        for (int k = 0; k < K; ++k) {
            sum += __half2float(A[m_idx * K + k]) * __half2float(B[n_idx * K + k]);
        }
...
def gemm_fp16_fp32_nt(a, b, out=None, alpha=1.0, beta=0.0):
    """
    Performs GEMM operation with FP16 inputs and FP32 accumulation.
    ...
    # Get kernel
    kernel_name = "gemm_fp16_fp32_nt"
    module = compiler.get_cached_module(
        FP16_FP32_GEMM_TEMPLATE,
        kernel_name=kernel_name,
        dtype=(torch.float16, torch.float16, torch.float32),
    )
...
def benchmark_fp16_fp32(m, n, k, num_warmups=5, num_runs=10):
    """
    Benchmark the FP16->FP32 GEMM operation.
